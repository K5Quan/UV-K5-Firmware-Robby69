...
//			else 
		...
		case KEY_5:
			if (beep) {
				if (IS_MR_CHANNEL(gTxVfo->CHANNEL_SAVE) && !gScanRangeStart) {APP_RunSpectrum(1);} //Channel scan
				if (gScanRangeStart) APP_RunSpectrum(3); //Range scan
				if (!IS_MR_CHANNEL(gTxVfo->CHANNEL_SAVE)) APP_RunSpectrum(4); //basic spectrum
			}
